...
The classical version of QERRA-v2 (Engine v1.9.1 / Pipeline Architecture v2.0.1) is the current working implementation of the SEMEV-12 framework. It operates both as an on-device robotics middleware and as a live public FastAPI service returning structured, explainable assessments.

The engine evaluates input text across all 12 vectors using semantic similarity against pre-encoded multi-anchor arrays via `all-MiniLM-L6-v2`. Five vectors (v004, v005, v010, v011, v012) apply regex pattern matching as supporting OR conditions for high-certainty phrases. The scoring includes contextual nuance logic: when high workplace pressure coincides with strong personal commitment and determination, the engine applies a calibrated dampening adjustment to avoid falsely penalizing professionals who are persevering through difficult environments.

Each activated vector contributes its assigned weight to a total ethical risk score (0.0 to 1.0). A score above 0.50 produces a `"modified"` decision, commanding the robot to hold its task, illuminate an amber LED, log an auditable refusal string, and physically shake its head "No" in simulation.

SEMEV-12 operates alongside a deterministic physical safety companion, **QERRA-HSR v0.1**. QERRA-HSR runs before SEMEV-12 on every control cycle in under 1 millisecond (0.0 ms command delay, <1 cm physical stop in simulation). An active `CRITICAL` reflex immediately clamps motor velocity to 0.0 rad/s and suspends SEMEV-12 deliberation. When conditions clear across a 1.0-second dwell cooldown, QERRA issues a `recovery_directive` separating routine tasks (cleared to auto-resume) from delicate tasks (holding position until a human confirms resumption).
