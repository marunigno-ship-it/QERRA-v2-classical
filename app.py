...
#
# Unified Pipeline:
#   POST /evaluate_pipeline connects all 3 layers in one end-to-end flow.
# =====================================================
...
class PipelineRequest(BaseModel):
    ...
    hsr_signals: Optional[HSRSignals] = None   # optional physical safety signals
...
def evaluate_pipeline(request: Request, data: PipelineRequest):
    ...
    2. Layer 3 (QERRA-THRIVE): Value-based Action Ranking to pick the best candidate action.
    3. Layer 1 (SEMEV-12): Moral deliberation gate evaluating the winning action.
    """
    # ── 1. LAYER 2: QERRA-HSR Physical Safety Check ──────────────────
    hsr_payload = None
    if data.hsr_signals is not None:
        hsr_input = HSRInput(
            distress_confidence=data.hsr_signals.distress_confidence,
    ...
    try:
        thrive_res = ranker_func(data.candidates)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Layer 3 ranking error: {str(e)}")

    winning_candidate = thrive_res.get("winner")
    thrive_fires = thrive_res.get("fires", True)

    # ── 3. LAYER 1: SEMEV-12 Moral Deliberation on Winning Action ───
    semev12_res = analyze_text(winning_candidate)
    semev12_decision = semev12_res.get("decision", "modified")

    # ── 4. Unified Synthesis & Final Decision ───────────────────────
    if not thrive_fires:
        # Layer 3 abstained (all candidates scored below quality threshold)
        pipeline_status = "ABSTAINED_LOW_VALUE"
        recommendation = "ask_human"
    elif semev12_decision == "modified":
        # Winning action violated moral/harm boundaries
        pipeline_status = "BLOCKED_MORAL_RISK"
        recommendation = "ask_human"
    else:
    ...
    return api_response({
        ...
        "chosen_action": winning_candidate if recommendation == "choose" else None,
        ...
        "layer_1_semev12": {
            "decision": semev12_decision,
            "score": semev12_res.get("score"),
            "vectors_activated": semev12_res.get("vectors_activated", []),
            "reasoning": semev12_res.get("reasoning", ""),
        }
    })
...
def home():
    return api_response({
        ...
        "endpoints": {
            ...
            "/evaluate_pipeline": "Unified 3-Layer end-to-end evaluation pipeline"
        }
    })
